...
func TestIdentifierExpression(t *testing.T) {
	...
	if len(program.Statements) != 1 {
		t.Fatalf("program doesn't have right amount of statements. got=%d expected=1",
			len(program.Statements))
	}
...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, tt := range prefixTests {
		...
		if len(program.Statements) != 1 {
			t.Fatalf("program not contain 1 statement. got=%d", len(program.Statements))
		}
...
func testIntegerLiteral(t *testing.T, intLiteral ast.Expression, value int64) bool {
	...
	if !ok {
		t.Errorf("intLiteral not *ast.IntegerLiteral. go=%T", intLiteral)
		return false
	}
